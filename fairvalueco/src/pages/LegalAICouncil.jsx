    ...
    <div className="min-h-screen bg-white text-primary">
      ...
      <main>
        ...
        <section id="capabilities" className="py-20 sm:py-28 bg-slate-50">
          <div className="max-w-7xl mx-auto px-6 sm:px-8"><div className="max-w-2xl"><p className="text-secondary font-bold tracking-[0.18em] uppercase text-sm">What FVA supports</p><h2 className="mt-4 text-3xl sm:text-4xl font-extrabold tracking-tight">A review layer built around evidence, not just text generation.</h2></div><div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">{capabilities.map(({ icon: Icon, title, text }) => <article key={title} className="rounded-xl border border-slate-200 bg-white p-6"><div className="h-11 w-11 rounded-lg bg-primary flex items-center justify-center"><Icon className="w-5 h-5 text-secondary" /></div><h3 className="mt-6 text-lg font-bold">{title}</h3><p className="mt-3 text-slate-600 leading-relaxed">{text}</p></article>)}</div></div>
        </section>
